...
export default function(){
       ...
       // if (!(updateStatus[item])){
            ...
            if (method[1].includes(item)){
              if (method[1].reduce(function(previous , current){if (composite[current]===undefined){return false}else{return previous}}, true)){
                composite[method[0].name] = await(method[0](composite ,compositeMetaData.grandParent ,callNumber));
  ...
  //  }
    if (callNumber != compositeMetaData.validAsyncCall) {
      ...
      if(Object.keys(compositeMetaData.insideFunctionUpdates)!=0){
...
// }
  ...
  const addFunction = function(){
    ...
    let injectingCodes = [
      ...
      // 'currentComposite[metaDataKey].validAsyncCall = Symbol();',
      // 'currentComposite[metaDataKey].options[item] = false;',
      // 'currentComposite[metaDataKey].runFunctions(currentComposite[metaDataKey] , currentComposite[metaDataKey].validAsyncCall );',
      // '}',
    ...
    if (arguments[1]) {
      ...
      for (let item of nestedPropertiesCourier.property){
        ...
        // let indexOfpropAddress = externalAddresses.indexOf(propAddress);
        // if (indexOfpropAddress != -1){
        //   if (externalAddresses[indexOfpropAddress].includes(affectedComposite)){
        //     // do nothing
        //   }else{
        //     externalAddresses[indexOfpropAddress].push([affectedComposite]);
        //   }
        // }else{
        //   externalAddresses[externalAddresses.push(propAddress) - 1].push([affectedComposite]);
        // }
      ...
      for (let item in varList){
        if (varList[item].length > 1){
          injectingCodes.push("let " + item + " = [];");
          for (let i=0;i<varList[item].length;++i){
            injectingCodes.push(item + "[" + i + "]" + varList[item][i]);
          }
        }else{
          injectingCodes.push("let " + item + varList[item][0]);
        }
      }
    }

    let functionString = method.toString();
    let paraString = method.toString().match(paraRegExp);
    ...
    functionBody =  injectingCodes.join('\n') + functionBody;
    if (paraString){
      paraString = paraString[1];
      let paraArray = paraString.split(',').map(item=>item.trim());
      paraArray.forEach(item => { functionPara.push(item)});
      paraString = "{" + paraString + "}";
    }else{
      paraString = "";
    }
    finalFunction = new Function(paraString , functionBody);
