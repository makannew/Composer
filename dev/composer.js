...
export default function(){
  ...
  //
  composite[metaDataKey]= {updateQueue:[], metaTree: {}, addressMap:new Map()};
  let metaTree = composite[metaDataKey].metaTree;
  let updateQueue = composite[metaDataKey].updateQueue;
  let addressMap = composite[metaDataKey].addressMap;
  addressMap.set(composite,new Address());
  ...
  const compositeHandler = {
    set: function ( obj , prop , value , receiver ){
      let addressRecorder =new Address(addressMap.get(obj).arr);
      ...
      if (typeof value === "object" && value != null){
        addressMap.set(obj[prop] , new Address(addressRecorder.arr));
      }
    ...
    get: function ( obj , prop , receiver ){
      let addressRecorder = addressMap.get(obj);
      if (addingLink || removingLink) {
        let propAddress = nestedPropertiesCourier.property;
        let lastPropAddress = propAddress[propAddress.length-1];
        if (obj[metaDataKey] && obj[metaDataKey].name == "courier"){
          if (!lastPropAddress.getRefFrom(composite)){
            composite[lastPropAddress.name()] = {};
            addressMap.set(composite[lastPropAddress.name()] , new Address(lastPropAddress.arr));
            buildMetaPath(lastPropAddress);
          }
          lastPropAddress.extend(prop);
        }else{
          propAddress.push(new Address(addressRecorder.arr));
          propAddress[propAddress.length-1].extend(prop);
        }
      ...
      if (typeof result === "object" && result != null){
        return new Proxy(result, compositeHandler);
      }
      return result;
    }
  }
